...
//         throw new Error(`User ${userId} not assigned to any schema`);
//       }
...
//   /**
//    * Get complete schema (BACKEND ONLY - not exposed as tool)
//    * Returns all tables, columns, and available fields in one call
//    */
...
//   /**
//    * Get rules for a field (BACKEND ONLY)
//    */
...
//       if (result.rows.length === 0) {
//         throw new Error(`No rules for field: ${fieldName}`);
//       }
...
//   /**
//    * Validate query (BACKEND ONLY - not exposed as tool)
//    */
...
//         const availableTables = existingTables.rows.map(r => r.table_name);
        
//         for (const table of tablesUsed) {
//           if (!availableTables.includes(table)) {
//             return { valid: false, error: `Table not found: ${table}` };
//           }
//         }
//       }
...
//   /**
//    * Execute query (only tool that actually queries)
//    */
//   async executeQuery(userId, query, limit = 100) {
//     try {
//       await this.getUserSchema(userId);

//       // Validate first (backend only, no tool call)
...
//       const result = await this.pool.query(limitedQuery, [limit]);
...
//   /**
//    * Build a query based on field and filters
//    * BACKEND ONLY - Helps AI build correct queries
//    */
//   async buildQueryForField(userId, fieldName, filters = {}) {
//     try {
...
//   /**
//    * Get all field suggestions for analysis
//    * BACKEND ONLY - Helps AI know what to query
//    */
...
// src/services/database.service.js - FIXED with proper search_path handling
import { Pool } from 'pg';
